...
def db_filter(dbs, httprequest=None):
    ...
    # Ajout OpenFire pour orienter vers la bonne base en fonction de l'URL
    if h.endswith('.openfire.fr'):
        base = h[:-12] # On prend comme nom de base tout ce qui précède ".openfire.fr" dans l'URL
        if base in dbs:
            return base
    # Fin ajout OpenFire
    d, _, r = h.partition('.')
